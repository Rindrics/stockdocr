...
test_that("pull given weight from vpa results", {
  ...
  draft  <- FALSE
  add_jp <- TRUE
  expect_equal(pull_value_from_vpa_result(res_vpa, varname = "biomass",
                                          yr = year),
               60380.74, tolerance = 0.01)
  expect_equal(pull_value_from_vpa_result(res_vpa, varname = "SSB",
                                      yr = year),
               21728.52, tolerance = 0.01)
  expect_equal(pull_value_from_vpa_result(res_vpa, varname = "catch",
                                          yr = year),
...
test_that("pull weight of given year from vpa results", {
  ...
  expect_equal(purrr::map_dbl(years, pull_value_from_vpa_result,
                              vpares = res_vpa, varname = "biomass") %>%
                 ton_to_kton() %>%
                 round(1) ,
...
test_that("ssb output in draft mode", {
  draft <- TRUE
  expect_equal(ssb(res_vpa, year = 2017,
...
test_that("harvest_rate", {
  expect_equal(harvest_rate(res_vpa,
